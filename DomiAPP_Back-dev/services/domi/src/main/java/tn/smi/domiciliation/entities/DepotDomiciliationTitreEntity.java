...
import tn.smi.domiciliation.UploadFile.FileEntity;
import tn.smi.domiciliation.entities.enumeration.*;
...
@NoArgsConstructor
@AllArgsConstructor
@jakarta.persistence.Table(name = "DEPOT_DOMICILIATION_TITRE", schema = "DOMI", catalog = "")
public class DepotDomiciliationTitreEntity implements Serializable {
    @Id
...
//    @EmbeddedId
//    private DepotDomiciliationTitreEntityPK depotDomiciliationTitreEntityPK;
    ...
    @Enumerated(EnumType.STRING)
    private RegimeSecteurActivite regimeSecteurActivite;

    @Basic
    @Column(name = "ACTIVITE_PRINCIPALE_SECONDAIRE")
    @Enumerated(EnumType.STRING)
    private ActivitePrincipaleSecondaire activitePrincipaleSecondaire;
    ...
    private long codPayAch;

    @Basic
    @Column(name = "COD_PAY_PROV")
    private long codPayProv;

    @Basic
    @Column(name = "COD_PAY_ORIG")
    private long codPayOrig;

    @Basic
    @Column(name = "CODE_PAYS_DEST_DEF")
    private long codePaysDestDef;
    ...
    @Enumerated(EnumType.STRING)
    private EtatVerif etatVerif=EtatVerif.En_Attente_De_Vérification;



    @OneToMany(cascade = CascadeType.ALL, mappedBy="depotdomiciliationtitreentity")
    private List<DetailPDepotEntity> detailPDepotEntitys;
...
//
//        DepotDomiciliationTitreEntity that = (DepotDomiciliationTitreEntity) o;
//
...
//        if (matEmp != null ? !matEmp.equals(that.matEmp) : that.matEmp != null) return false;
...
//        result = 31 * result + (matEmp != null ? matEmp.hashCode() : 0);
